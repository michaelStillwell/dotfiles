...
$env.config = {
    ...
    color_config: $gruvbox_theme # if you want a more interesting theme, you can replace the empty record with `$dark_theme`, `$light_theme` or another custom record
    use_grid_icons: true
...
def proj [] {
	echo 'Need to implement this again'
}

# Zellij stuffs

# open a session or create random one
def zl [session_name?: string] {
...
def zr [command: string, ...args] {
	zellij run --name $command -- $command ...$args
}

def zrf [command: string, ...args] {
	zellij run --name $command --floating -- $command ...$args
}

# TODO: figure out why this crashes, in place crashes in nushell?
#def zri [command: string, ...args] {
	#zellij run --name $command --in-place -- $command $args
#}

# TODO: this isn't running, but looks to not work in fish either?
def ze [file_name: string] {
	zellij edit $file_name
}

# TODO: this isn't running, but looks to not work in fish either?
def zef [file_name: string] {
	zellij edit --floating $file_name
}
